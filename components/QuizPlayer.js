...
import htm from 'htm';
import { submitQuiz, getCurrentUser } from '../services/mockBackend.js';
            ...
            ${currentQuestion.options.map((option, idx) => {
              ...
              if (isAnswered) {
                if (idx === currentQuestion.correctAnswerIndex) {
                  ...
                  icon = html`<${CheckCircle} className="text-emerald-500 fill-emerald-100 dark:fill-emerald-900" size=${24} />`;
                } else if (idx === selectedOption) {
                  optionClass += "border-rose-500 bg-rose-50 dark:bg-rose-900/20 text-rose-900 dark:text-rose-300 shadow-sm";
                  icon = html`<${XCircle} className="text-rose-500 fill-rose-100 dark:fill-rose-900" size=${24} />`;
                } else {
